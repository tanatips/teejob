...
class Cls_Shipment_Models {
    ...
    //=== ---- Connect MQTT Broker ---- ===//
    Fn_SendMqttPrivate(toppicRequest, jsonStr) {

        let brokerPrivate = myGlobal.Mqtt.connect({
            host: myGlobal.ItemConfig.BROKER_PRIVATE.HOST,
            port: myGlobal.ItemConfig.BROKER_PRIVATE.PORT,
            username: myGlobal.ItemConfig.MQTT_AUTHEN_PRIVATE.USER_NAME,
            password: myGlobal.ItemConfig.MQTT_AUTHEN_PRIVATE.PASSWORD
        });

        brokerPrivate.on('connect', function () {

            //console.log("Data from web API Ok, Return: " + dataItem.ACTION_RETURN);

            //=== publisher to Public ===//
            brokerPrivate.publish(toppicRequest, Buffer.from(jsonStr, 'utf8'), myGlobal.Options, function (error) {
